...
-- Системные хеши префабов устройств игры
...
-- Атмосферные и температурные профили 8 комнат базы
local rooms_cfg = {
    { id = 1, name = "Тамбур",            is_greenhouse = false, min_p = 100, max_p = 105, target_t = 293.15 },
    { id = 2, name = "Производственная",  is_greenhouse = false, min_p = 101, max_p = 106, target_t = 293.15 },
    { id = 3, name = "Плавки металлов",   is_greenhouse = false, min_p = 95,  max_p = 100, target_t = 293.15 }, 
    { id = 4, name = "Жилая комната",     is_greenhouse = false, min_p = 101, max_p = 106, target_t = 293.15 }, 
    { id = 5, name = "Аппаратная связи",  is_greenhouse = false, min_p = 100, max_p = 105, target_t = 291.15 }, 
    { id = 6, name = "Комната ACCS",      is_greenhouse = false, min_p = 100, max_p = 105, target_t = 291.15 }, 
    { id = 7, name = "Теплица (CO2)",     is_greenhouse = true,  min_p = 80,  max_p = 85,  target_t = 298.15 }, 
    { id = 8, name = "Фильтрация газов",  is_greenhouse = false, min_p = 90,  max_p = 95,  target_t = 293.15 }
}

-- Динамические привязки устройств
local accs_devices = {}
local accs_data = {}
local dropdown_open = {}
...
-- ======================================================================
-- БЕЗОПАСНЫЕ ФУНКЦИИ API И ДИНАМИЧЕСКИЙ СБОР УСТРОЙСТВ
-- ======================================================================
...
-- Сбор всех подключённых приборов по их prefab_hash
...
-- ======================================================================
-- ОПРОС ГАЗОВ И ИСПОЛНИТЕЛЬНАЯ АВТОМАТИКА ACCS
-- ======================================================================
...
local function refresh_accs_readings()
    ...
    for i = 1, BOX_COUNT do
        ...
        local prefab = tonumber(dev.prefab) or 0
        local namehash = tonumber(dev.namehash) or 0

        if prefab ~= 0 and namehash ~= 0 then
            local status, err = pcall(function()
                local p = safe_batch_read_name(prefab, namehash, LT.Pressure, LBM.Average)
                
                if p and p > 0 then
                    accs_data[i].pressure    = p
                    accs_data[i].temperature = safe_batch_read_name(prefab, namehash, LT.Temperature, LBM.Average) or 293.15
                    
                    -- Химический спектр газов
                    accs_data[i].ratio_o2    = safe_batch_read_name(prefab, namehash, LT.RatioOxygen, LBM.Average) or 0
                    accs_data[i].ratio_n2    = safe_batch_read_name(prefab, namehash, LT.RatioNitrogen, LBM.Average) or 0
                    accs_data[i].ratio_co2   = safe_batch_read_name(prefab, namehash, LT.RatioCarbonDioxide, LBM.Average) or 0
                    accs_data[i].ratio_pol   = safe_batch_read_name(prefab, namehash, LT.RatioPollutant, LBM.Average) or 0
                    accs_data[i].ratio_n2o   = safe_batch_read_name(prefab, namehash, LT.RatioNitrousOxide, LBM.Average) or 0
                    accs_data[i].ratio_h2o   = safe_batch_read_name(prefab, namehash, LT.RatioWater, LBM.Average) or 0
                    accs_data[i].ratio_ch4   = safe_batch_read_name(prefab, namehash, LT.RatioMethane, LBM.Average) or 0
                    accs_data[i].ratio_h2    = safe_batch_read_name(prefab, namehash, LT.RatioHydrogen, LBM.Average) or 0
        ...
        -- Глобальный статус всей сети
        if accs_data[i].room_status == STATE_CRITICAL then
            base_global_status = STATE_CRITICAL
        elseif accs_data[i].room_status == STATE_WARNING and base_global_status ~= STATE_CRITICAL then
...
local function execute_accs_logic()
    for i = 1, BOX_COUNT do
        ...
        if data.is_online then
            ...
            -- КЛИМАТ-КОНТРОЛЬ НА КЛАПАНАХ (КРОМЕ ТЕПЛИЦЫ i=7)
            if i ~= 7 and tonumber(v_valve.prefab) ~= 0 and tonumber(v_valve.namehash) ~= 0 then
            ...
            safe_batch_write_name(v_in.prefab, v_in.namehash, LT.On, 0)
            safe_batch_write_name(v_out.prefab, v_out.namehash, LT.On, 0)
            if i ~= 7 then safe_batch_write_name(v_valve.prefab, v_valve.namehash, LT.On, 0) end
        end
    end
end
-- ======================================================================
-- ПРОМЫШЛЕННЫЙ ИНТЕРФЕЙС ВКЛАДКИ ОБЗОР (ЧАСТЬ 4.1: ШАПКА МАТРИЦЫ БАЗЫ)
-- ======================================================================
...
local function render_overview()
    ...
    -- ИСПРАВЛЕНИЕ: Увеличенная высота строк до 24px для заполнения нижних 2/5 экрана
    local start_x = 10
    local start_y = 54   
    local row_h   = 44   
    local border_gap = 2

    -- ИСПРАВЛЕНИЕ: Суженная колонка названий для высвобождения места под крупные цифры
    local col_room_w = 70  -- Название помещения
    ...
    -- ИСПРАВЛЕНИЕ: Добавлены единицы измерения (кПа, °C, %) в шапку таблицы и увеличен шрифт до 11
    s:element({ id = "h_col_room", type = "label", rect = { unit = "px", x = x_room, y = start_y, w = col_room_w, h = 14 }, props = { text = "ЗОНА" }, style = { font_size = 11, color = C.accent, align = "left" } })
...
-- ======================================================================
-- ПРОМЫШЛЕННЫЙ ИНТЕРФЕЙС ВКЛАДКИ ОБЗОР (ЧАСТЬ 4.2: ИДЕАЛЬНЫЙ ЦЕНТР 44PX)
-- ======================================================================
    ...
    for i = 1, BOX_COUNT do
        ...
        s:element({
            id = "row_panel_" .. i, type = "panel",
            rect = { unit = "px", x = start_x, y = cur_y, w = W - 20, h = row_h - 2 },
            style = { bg = C.panel, border_color = "#121A30", border_width = 1 }
        })

        -- ВАШЕ ИДЕАЛЬНОЕ ИСПРАВЛЕНИЕ: Центрированный перенос длинных имён
        local first_word, second_word = string.match(cfg.name, "([^%s]+)%s+(.*)")
        if first_word and second_word then
            -- Смещение y изменено на +11 и +23 для идеального центра в ячейке 44px
            ...
            -- Смещение y изменено на +15 для одиночных названий
        ...
        -- КОРРЕКЦИЯ ЦЕНТРА: Смещение показателей по y изменено с +4 на +14 (для высоты ячейки 44px)
        ...
        -- КОРРЕКЦИЯ ЦЕНТРА: Смещение Danger Matrix и бокса OK/OFF по y также выровнено на +14
        if data.is_online then
            local alert_tag = ""
            ...
            elseif data.ratio_h2o > 0.001 then alert_tag = "H2O"
            end

            if alert_tag ~= "" then
                local b_w, b_h = 42, 16
                local b_x = x_dng + math.floor((col_dng_w - b_w) / 2)
                local b_y = cur_y + 14 -- Выравнивание бокса по центру

                s:element({ id = "alert_box_" .. i, type = "panel", rect = { unit = "px", x = b_x, y = b_y, w = b_w, h = b_h }, style = { bg = (elapsed % 2 == 0) and C.red or "#40060B" } })
                s:element({ id = "alert_txt_" .. i, type = "label", rect = { unit = "px", x = b_x, y = b_y + 2, w = b_w, h = b_h - 2 }, props = { text = alert_tag }, style = { font_size = 8, color = "#FFFFFF", align = "center" } })
            else
...
-- ======================================================================
-- ПРОМЫШЛЕННЫЙ ИНТЕРФЕЙС НАСТРОЕК (ЧАСТЬ 5.1: УСТАНОВКИ КЛИМАТА)
-- ======================================================================
...
local function render_settings()
    ...
    -- Главный заголовок меню конфигурации
    ...
    -- Индекс настраиваемой комнаты (settings_page от 1 до 8)
    if settings_page < 1 or settings_page > BOX_COUNT then settings_page = 1 end
    local idx = settings_page
    local cfg = rooms_cfg[idx]

    -- Панель-контейнер для настроек
    ...
    -- СЕЛЕКТОР ВЫБОРА КОМНАТЫ: Выпадающий список взамен перелистывания страниц
    ...
    s:element({
        ...
        on_change = function(opt)
            ...
            settings_room_open = "false"
            render_settings() draw_navigation_tabs()
        end
    })

    local item_start_y = pane_y + 42
    local step_y = 23
    local label_w, input_x, drop_w = 170, pane_x + 190, 240

    -- ИСПРАВЛЕНИЕ: Удалены цифры 2,3,4 и увеличен шрифт до 14 (Установка ТЕМПЕРАТУРЫ)
    if idx ~= 7 then
        s:element({ id = "t_cfg_lbl", type = "label", rect = { unit = "px", x = pane_x + 14, y = item_start_y + 2, w = label_w, h = 16 }, props = { text = "Настройка температуры:" }, style = { font_size = 11, color = C.text, align = "left" } })
        s:element({ id = "t_minus_b", type = "button", rect = { unit = "px", x = input_x, y = item_start_y, w = 18, h = 18 }, props = { text = "-" }, style = { bg = "#1E2538", text = C.text, font_size = 11 }, on_click = function() cfg.target_t = math.max(273.15, cfg.target_t - 1.0) render_settings() draw_navigation_tabs() end })
        
        local current_t_c = math.floor(cfg.target_t - 293.15 + 0.5)
        s:element({ id = "t_val_lbl", type = "label", rect = { unit = "px", x = input_x + 22, y = item_start_y + 2, w = 55, h = 16 }, props = { text = string.format("%d °C", current_t_c) }, style = { font_size = 14, color = C.yellow, align = "center" } })
        s:element({ id = "t_plus_b", type = "button", rect = { unit = "px", x = input_x + 81, y = item_start_y, w = 18, h = 18 }, props = { text = "+" }, style = { bg = "#1E2538", text = C.text, font_size = 11 }, on_click = function() cfg.target_t = math.min(323.15, cfg.target_t + 1.0) render_settings() draw_navigation_tabs() end })
    else
        s:element({ id = "t_cfg_lbl_gh", type = "label", rect = { unit = "px", x = pane_x + 14, y = item_start_y + 2, w = 300, h = 16 }, props = { text = "Климат теплицы автономен" }, style = { font_size = 11, color = C.text_dim, align = "left" } })
    end

    -- ИСПРАВЛЕНИЕ: Удалены цифры и увеличен шрифт до 14 (Установка ДАВЛЕНИЯ)
    local y_p = item_start_y + step_y
    ...
    s:element({ id = "p_minus_b", type = "button", rect = { unit = "px", x = input_x, y = y_p, w = 18, h = 18 }, props = { text = "-" }, style = { bg = "#1E2538", text = C.text, font_size = 11 }, on_click = function() cfg.min_p = math.max(0, cfg.min_p - 5) cfg.max_p = cfg.min_p + 5 render_settings() draw_navigation_tabs() end })
    s:element({ id = "p_val_lbl", type = "label", rect = { unit = "px", x = input_x + 22, y = y_p + 2, w = 55, h = 16 }, props = { text = string.format("%d kPa", cfg.min_p) }, style = { font_size = 14, color = C.accent, align = "center" } })
    s:element({ id = "p_plus_b", type = "button", rect = { unit = "px", x = input_x + 81, y = y_p, w = 18, h = 18 }, props = { text = "+" }, style = { bg = "#1E2538", text = C.text, font_size = 11 }, on_click = function() cfg.min_p = math.min(200, cfg.min_p + 5) cfg.max_p = cfg.min_p + 5 render_settings() draw_navigation_tabs() end })
-- ======================================================================
-- ПРОМЫШЛЕННЫЙ ИНТЕРФЕЙС НАСТРОЕК (ЧАСТЬ 5.2: ФИКС CABLE-СЕЛЕКТОРОВ)
-- ======================================================================

    -- Селектор Датчика газа
    ...
    s:element({
        ...
        -- ИСПРАВЛЕНИЕ БАГА 427: Использовано верное свойство d.name_hash вместо ошибочного d.namehash
        on_change = function(opt)
    ...
    -- Селектор Клапана Радиатора (Если не Теплица №7)
    if idx ~= 7 then
...
-- ======================================================================
-- НАВИГАЦИЯ, ФИЗИЧЕСКИЙ ЭКРАН И ГЛАВНАЯ ТОЧКА ВХОДА СИСТЕМЫ
-- ======================================================================

draw_navigation_tabs = function()
    -- Кнопка перехода во вкладку ОБЗОР (Укрупнена и опущена ниже заголовка)
    s:element({ 
        id = "tab_overview", type = "button", 
        rect = { unit = "px", x = 12, y = 28, w = 100, h = 20 }, 
        props = { text = "ОБЗОР" }, 
        style = { 
            bg = view == "overview" and "#1E3A8A" or C.panel, 
            text = "#FFFFFF", font_size = 10, 
            border_color = view == "overview" and C.accent or "#1E2538", border_width = 1 
        }, 
        on_click = function() 
            view = "overview" 
            s = surfaces.overview 
    ...
    -- Кнопка перехода во вкладку НАСТРОЙКИ (Укрупнена и опущена ниже заголовка)
    s:element({ 
        id = "tab_settings", type = "button", 
        rect = { unit = "px", x = 122, y = 28, w = 100, h = 20 }, 
        props = { text = "НАСТРОЙКИ" }, 
        style = { 
            bg = view == "settings" and "#1E3A8A" or C.panel, 
            text = "#FFFFFF", font_size = 10, 
            border_color = view == "settings" and C.accent or "#1E2538", border_width = 1 
        }, 
        on_click = function() 
            view = "settings" 
            s = surfaces.settings 
...
local function update_hardware_console()
    ...
    -- Так как экран "Обзор" теперь матричный, физическая подложка экрана 2х2 в Тамбуре
    -- всегда отражает критическое состояние базы (base_global_status).
    -- Если везде норма — горит зеленым (2), если дисбаланс — желтым (1), при аварии — красным (4).
    local hardware_color = 0
    if base_global_status == STATE_NOMINAL then hardware_color = 2
    elseif base_global_status == STATE_WARNING then hardware_color = 1
    elseif base_global_status == STATE_CRITICAL then hardware_color = 4 end

    safe_batch_write_name(CONSOLE_2X2_HASH, name_hash, LT.Color, hardware_color)
    
    -- На график физической консоли выводим усредненное или пиковое давление по всей базе
    -- Для наглядности выведем давление из первой комнаты (Тамбура) или текущей выбранной в настройках
    local disp_p = accs_data[active_display_room].is_online and math.floor(accs_data[active_display_room].pressure) or 0
    safe_batch_write_name(CONSOLE_2X2_HASH, name_hash, LT.Mode, disp_p)
end
-- ======================================================================
-- ЭНЕРГОНЕЗАВИСИМАЯ ПАМЯТЬ: СОХРАНЕНИЕ И ЗАГРУЗКА (ИЗ ПРОЕКТА PIAC)
-- ======================================================================
...
while true do
    ...
    if refresh_tick >= LIVE_REFRESH_TICKS then
        refresh_accs_readings() 
        execute_accs_logic() 
        update_hardware_console()
        
        -- Постоянно обновляем матрицу всей базы на экране "ОБЗОР"
